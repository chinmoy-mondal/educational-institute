...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const subOptions = {
        Exam: ['Half Yearly Exam', 'Annual Exam', 'Pre-Test Exam', 'Test Exam'],
        Holiday: ['Independence Day', 'Victory Day', 'Pohela Boishakh', 'Eid-ul-Fitr', 'Eid-ul-Adha',
            'Christmas Day'
        ],
        Notice: ['General Notice', 'Exam Notice', 'Holiday Notice'],
        Vacation: ['Summer Vacation', 'Winter Vacation']
    ...
    document.getElementById('add-category')?.addEventListener('change', e => handleCategoryChange(e.target
        .value, 'add'));
    document.getElementById('edit-category')?.addEventListener('change', e => handleCategoryChange(e.target
        .value, 'edit'));
    ...
    function filterSubjectOptions(classSelectorId, subjectSelectorId) {
        ...
        const subjEl = document.getElementById(subjectSelectorId);
        Array.from(subjEl.options).forEach(opt => {
            opt.style.display = (!opt.value || !opt.dataset.class || opt.dataset.class == classVal) ?
                '' : 'none';
        });
    }

    document.getElementById('add-class')?.addEventListener('change', () => filterSubjectOptions('add-class',
        'add-subject'));
    document.getElementById('edit-class')?.addEventListener('change', () => filterSubjectOptions('edit-class',
        'edit-subject'));
    ...
    document.getElementById('deleteEvent')?.addEventListener('click', async function() {
        ...
        const params = new URLSearchParams();
        params.append('id', id);
        Array.from(document.querySelectorAll('#editEventForm input[type="hidden"]')).forEach(i => i
            .name && params.append(i.name, i.value));
        const result = await fetchJson('/calendar/delete', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
